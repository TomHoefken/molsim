module CUDAModule
    ...
    !  real(8),device, allocatable  :: deltaEn(:)
      ...
      integer(4)         :: numblocks1
      integer(4)         :: numblocks2
      ...
      logical :: lliang
      logical,device :: lliang_d
      logical :: lmcpasscuda
      logical,device :: lmcpasscuda_d
     ...
     integer(kind=cuda_stream_kind) :: stream6
      ...
      subroutine GPUDriver(iStage)
         ...
         namelist /nmlCUDA/ lliang, lmcpasscuda
...
! ... allocate memory for arrays on GPU
      ...
      subroutine MCPassCUDA
         ...
         if (lliang) then
            ...
            call MCCudaUpdate_liang
            lmcpass = .false.
         else if (lmcpasscuda) then
      ...
      end subroutine MCCudaUpdate_liang

      subroutine MCCudaUpdate_MCPass
      ...
      end subroutine MCCudaUpdate_MCPass
         !! subroutine MCPass
      ...
      subroutine MCPassAllGPU
         ...
         integer(4) :: i,j,istat,ierr,ierra
               ...
               do j = 1, iloops
                  ...
                  ierra = cudaDeviceSynchronize()
               !if (lweakcharge) call TransferToAux<<<iblock1,256>>>
                call TransferToAux<<<iblock1,256>>>
                  ierra = cudaDeviceSynchronize()
     ...
     attributes(global) subroutine GenerateRandoms
      ...
      attributes(global) subroutine CalculateUpperPart(lhsoverlap)
         ...
         real(fp_kind), shared   :: iptpt_s(16,16)
         real(fp_kind), shared   :: ucoff_s(16)
         integer(4) :: ibuf
               ...
               if (id <= np_s) then
                  ...
                  if ( id_int <= npt_s) then
                     do i=1, npt_s
                        ...
                        iptpt_s(id_int,i) = iptpt_d(id_int,i)
                        ucoff_s(iptpt_d(id_int,i)) = ucoff_d(iptpt_d(id_int,i))
                     end do
                  end if
               end if
             ...
             do j =blockIDx%x, numblocks
                  ...
                  do i=1, blocksize
                     ...
                     if (jp <= np_s) then
                        ...
                        !usum = ucoff_s(iptpt_s(iptip_s,iptjp_s(i)))/sqrt(rdist)
...
! TODO: set flags for lcharge and lweakcharge
! TODO: improve data transfers; perform asynchronous transfers
      ...
      ipt = iptpn_d(ip)
        if ( ip /= jp ) then
             ...
             if (.not. lptm_d(jp)) then
              ...
              ! EllipsoidOverlap and SuperballOverlap currently not supported
              ...
              if (r2 > rcut2_d) then
                !do not anything
                 ...
                 usum1(tidx_int) = ubuf_d(ibuf+1)+d*(ubuf_d(ibuf+2)+d*(ubuf_d(ibuf+3)+ &
                              d*(ubuf_d(ibuf+4)+d*(ubuf_d(ibuf+5)+d*ubuf_d(ibuf+6)))))
                 !usum1(tidx_int) = ucoff_d(iptjpt)/sqrt(r2)
              ...
              usum2(tidx_int) = ubuf_d(ibuf+1)+d*(ubuf_d(ibuf+2)+d*(ubuf_d(ibuf+3)+ &
                 ...
                 !usum2(tidx_int) = ucoff_d(iptjpt)/sqrt(r2)
                 !usum2(tidx_int) = usum2(tidx_int) + 4*ucoff_d(iptjpt)*((ucoff_d(iptjpt)/r2**6)-(ucoff_d(iptjpt)/r2**3))
                 !       usum1(tidx_int) = usum1(tidx_int) - usum2(tidx_int)
        end if
     end if
...
attributes(global) subroutine UTwoBodyAAll_weak(lhsoverlap)
...
end subroutine UTwoBodyAAll_weak
...
attributes(grid_global) subroutine MCPass_cuda
   ...
   real(fp_kind) :: beta_s, fac_metro, dured, dutot_s
         ...
         if (tidx <= np_s) then
            ipt = iptpn_d(n)
              if ( jp /= n ) then
                   ...
                   !              d*(ubuf_d(ibuf+4)+d*(ubuf_d(ibuf+5)+d*ubuf_d(ibuf+6)))))
                   ! usum1(tidx_int) = usum1(tidx_int) - usum2(tidx_int)
                   call StartCalcUTwob_MCPass(n,jp,usum1(tidx_int),usum2(tidx_int),r2new,r2old,iptjpt)
              end if
           end if
        ...
        400 continue
         ...
         !reduction of dutwobody
          ...
          if (tidx_int == 1) then
             ...
             do i = 1, nptpt_d
                istat = atomicAdd(dutwob_d(i),usum_aux1(1,i))
             end do
          end if
...
attributes(device) subroutine StartCalcUTwob_MCPass(n,jp,usum1,usum2,r2new,r2old,iptjpt)
   ...
   integer(4) :: icounterion, jpweak, iptjpt2
   logical :: lcharged


   if (lweakcharge_d) then
      if (jp == iananweakcharge_d(n) .and. laztm_d(jp)) then
         lcharged = .true.
      else if (iananweakcharge_d(n) /= jp .and. laz_d(jp)) then
         lcharged = .true.
      else
         lcharged = .false.
      end if
      if (laztm_d(n) .and. lcharged) then
         call CalcUTwobnew_MCPass(n,jp,usum1,r2new,iptjpt)
         icounterion = iananweakcharge_d(n)
         if (iananweakcharge_d(n) /= 0) then
            if (jp == icounterion) then
               jpweak = n
               iptjpt2 = iptpt_d(iptpn_d(icounterion),iptpn_d(jpweak))
            else
               jpweak = jp
            end if
            iptjpt2 = iptpt_d(iptpn_d(icounterion),iptpn_d(jpweak))
            call CalcUTwobnew_MCPass(icounterion,jpweak,usum1,r2new,iptjpt2)
         end if
      end if
   ...
   if (lweakcharge_d) then
      if (laz_d(n) .and. laz_d(jp)) then
         call CalcUTwobold_MCPass(n,jp,usum1,usum2,r2old,iptjpt)
         if (iananweakcharge_d(n) /= 0) then
            call CalcUTwobold_MCPass(icounterion,jpweak,usum1,usum2,r2old,iptjpt2)
         end if
      end if
   else if (lcharge_d) then
      call CalcUTwobold_MCPass(n,jp,usum1,usum2,r2old,iptjpt)
...
attributes(device) subroutine Metropolis_MCPass(dutot,beta,ip)
   ...
   if (lhsoverlap_d == .true.) then
      ...
      if (idecision == 1) then
            ...
            !utot_d = utot_d + E_s
            laz_d(ip) = laztm_d(ip)
            if (iananweakcharge_d(ip) /= 0) then
               laz_d(iananweakcharge_d(ip)) = laz_d(ip)
            end if
   ...
   if (lweakcharge_d) then
      laztm_d(ip) = laz_d(ip)
      if (iananweakcharge_d(ip) /= 0) then
         laztm_d(iananweakcharge_d(ip)) = laz_d(ip)
      end if
   end if
...
subroutine AllocateDeviceParams
   ...
   if(ltime) call CpuAdd('start', 'allocation', 1, uout)
        ...
        allocate(utwobnew_d(nptpt))
        allocate(utwobnew(nptpt))
        utwobnew = Zero
...
subroutine TransferConstantParams
   ...
   if(ltime) call CpuAdd('stop', 'transferconstant', 1, uout)
        ...
        istat = cudaStreamCreate(stream6)
...
subroutine CalcNumblocks

   implicit none
   integer(4) :: sizeofblocks =512

   numblocks1 = floor(Real((1*np)/sizeofblocks)) + 1
   numblocks2 = floor(Real((2*np)/sizeofblocks)) + 1
   isharedmem = 2*sizeofblocks*fp_kind + sizeofblocks*4 + threadssum*(nptpt+1)*fp_kind + 4*sizeofblocks


end subroutine CalcNumblocks
